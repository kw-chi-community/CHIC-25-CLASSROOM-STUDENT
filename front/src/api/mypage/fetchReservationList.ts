import { fetchReservationListDto } from "./dto/fetchReservationListDto";

export const fetchReservationList = async (studentId: string) => {
  try {
    ...
    const data: fetchReservationListDto[] = await response.json(); // JSON 형태로 응답 데이터 파싱
    return data;
  } catch (error) {
